...
void CDlgBookSale2::OnChangeBarcode() 
{
	if (changed)
	{
		...
		{
			CString sql;
			sql.Format("select a.barcode,a.bookname,a.author,b.sellprice from tb_bookinfo a inner join tb_fixprice b on a.barcode = b.barcode and (a.barcode = '%s' or a.bookname like '*%s*' or a.shortname = '%s' )",str,str,str);
			m_pRs->raw_Close();
...
float CDlgBookSale2::CalculateMoney()
{
...
void CDlgBookSale2::OnChangeRebate() 
{
...
void CDlgBookSale2::OnChangeSummoney() 
{
	//Invoke Rebate EditBox OnChangeEvent
	OnChangeRebate();
}
//删除列表数据
void CDlgBookSale2::DeleteListData()
{
...
bool CDlgBookSale2::HeaderInfoIsNull()
{
...
bool CDlgBookSale2::ListInfoIsNull()
{
...
void CDlgBookSale2::AddOperate()
{
	...
	m_pCom->CommandText = "sp_booksell";
	...
	try
	{
		...
		for(int row = 0; row< m_list.GetItemCount();row++)
		{
			...
			m_pRs->raw_Close();
			CString sql;
			sql.Format("insert into tb_sell_sub values ('%s','%s',%f,%f,%f,%f)",c_id,c_barcode,f_unitprice,f_number,f_rebate,f_money);
			m_pRs->Open((_variant_t)sql,m_pCon.GetInterfacePtr(),adOpenKeyset,adLockOptimistic,adCmdText);
...
void CDlgBookSale2::OnAdd() 
{
	AddOperate();
	
}

void CDlgBookSale2::OnCancel() 
{
	CancelOperate();
	
}

void CDlgBookSale2::OnExit() 
{
	CDialog::OnCancel();	
}
